...
try:
    ...
    from models.alphabet import Alphabet, gene_prot_point_accepted_alignment_matrix_prob
    ...
    from src.utils import set_seed, to_device, get_labels, get_parameter_number, save_model_parameters, \
        download_trained_checkpoint_lucaon_v1, download_trained_checkpoint_lucavirus
    ...
    from src.models.alphabet import Alphabet, gene_prot_point_accepted_alignment_matrix_prob
...
def get_args():
    parser = argparse.ArgumentParser(description='LucaVirus')
    # for logging
    parser.add_argument(
        "--tb_log_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="the hidden size (embedding vector size)"
    )
    parser.add_argument(
        "--num_attention_heads",
        type=int,
    ...
    parser.add_argument(
        "--tokenization",
        action="store_true",
        help="whether to use tokenization"
    )
    parser.add_argument(
        "--tokenizer_dir",
        default=None,
        type=str,
        help="the pretrained tokenizer info path(subword-level)"
    )
    parser.add_argument(
        "--vocab_path",
        default=None,
        type=str,
        help="vocab path(char-level)"
    )
    parser.add_argument(
        '--add_special_tokens',
        action='store_true',
        help='add special tokens in the start and end position([CLS], [SEP]) of the input sequence'
    )
    parser.add_argument(
        '--padding',
        default='right',
    ...
    parser.add_argument(
        ...
        default=0.0,
        required=True,
        help="the dropout prob for default"
    )
    parser.add_argument(
        '--attention_dropout_prob',
        type=float,
        default=None,
        help="the dropout prob for attention probs"
    )
    parser.add_argument(
        '--activation_dropout_prob',
        type=float,
        default=None,
        help="the dropout prob for FFN activate function output"
    )
    parser.add_argument(
        '--hidden_dropout_prob',
        type=float,
        default=None,
        help="the dropout prob for hidden layer"
    )
    parser.add_argument(
        '--pooler_dropout_prob',
        type=float,
        default=None,
        help="the dropout prob for pooler layer"
    )
    parser.add_argument(
        '--classifier_dropout_prob',
        type=float,
        default=None,
        help="the dropout prob for classification layer"
    )
    # pooling_type
    parser.add_argument(
        "--pooling_type",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="the evaluation dataset dir path."
    )
    parser.add_argument(
        "--test_data_dir",
        default=None,
        type=str,
        required=True,
        help="the test dataset dir path."
    )
    ...
    parser.add_argument("--non_ignore", type=str, default=None,
                        help="none ignore tasks.")
    parser.add_argument("--gene_mask_loss_type", type=str, default="cce",
                        choices=[ "cce", "soft_cce"],
    ...
    parser.add_argument("--prot_secondary_loss_type", type=str, default="cce",
                        choices=["cce"],
    ...
    parser.add_argument('--buffer_size', default=10240, type=int,
    ...
    parser.add_argument("--pretrain_task_level_type", default="all", type=str, required=True,
                        help="pre train task level type")
    parser.add_argument("--pretrain_task_level_name",
                        default="gene_mask,gene_type,gene_taxonomy,"
                                "prot_mask,prot_site,prot_domain,prot_homo,prot_taxonomy,prot_keyword,prot_structure,"
                                "trans",
    ...
    parser.add_argument(
        ...
        default=0,
        type=float,
        help="the global total loss to continue training"
    )
    parser.add_argument(
        "--cur_epoch_loss",
        default=0,
        type=float,
        help="the cur epoch loss to continue training"
    )
...
def check_args(args):
    ...
    # 下载lucaone checkpoint作为初始化，或者加载lucavirus checkpoint作为继续训练
    if args.model_dirpath:
        ss = args.model_dirpath.split("/")
        llm_type = None
        llm_version = None
        llm_task_level = None
        llm_time_str = None
        ...
        for s_idx, s in enumerate(ss):
            if "checkpoint-step" in s:
                ...
                llm_time_str = ss[s_idx - 1]
                llm_type = ss[s_idx - 2]
                llm_task_level = ss[s_idx - 3]
                llm_version = ss[s_idx - 4]
                break
        if llm_type is None:
    ...
    # for soft_cce of mask tasks
    if args.gene_mask_loss_type == "soft_cce" or args.prot_mask_loss_type == "soft_cce":
        args.label_relations = np.copy(gene_prot_point_accepted_alignment_matrix_prob)
    loss_type = {}
    output_mode = {}
    gene_output_keys = {}
    ...
    if args.non_ignore:
        ...
        for s in non_ignore_values:
            assert s.strip() in [
                "gene_mask", "prot_mask", "gene_type", "gene_taxonomy",
...
def get_model(args):
    ...
    if args.attention_dropout_prob is None:
        ...
        model_config.attention_probs_dropout = args.attention_dropout_prob
    ...
    if args.hidden_dropout_prob is None:
        ...
        model_config.hidden_dropout = args.dropout_prob
    else:
        model_config.hidden_dropout_prob = args.hidden_dropout_prob
        model_config.hidden_dropout = args.hidden_dropout_prob

    if args.pooler_dropout_prob is None:
        model_config.pooler_dropout_prob = args.dropout_prob
        model_config.pooler_dropout = args.dropout_prob
    else:
        model_config.pooler_dropout_prob = args.pooler_dropout_prob
        model_config.pooler_dropout = args.pooler_dropout_prob

    if args.classifier_dropout_prob is None:
        ...
        model_config.classifier_dropout = args.classifier_dropout_prob
...
def main():
    ...
    if args.local_rank in [0, -1]:
        ...
        if "label_relations" in args_dict and args_dict["label_relations"] is not None:
            args_dict["label_relations_diag"] = str(np.diag(args_dict["label_relations"]).tolist())
            args_dict["label_relations_shape"] = str(args_dict["label_relations"].shape)
            args_dict["label_relations"] = str(args_dict["label_relations"].tolist())
        print("#" * 20 + "args:" + "#" * 20)
        print(args_dict)
        print("#" * 50)
