# sh
# all tasks(3 + 6) continue training, when failure
# the dataset contain the entire dataset(gene+prot)
# 数据集信息
# 数据集
DATASET_NAME="lucavirus"
# 版本号
DATASET_TYPE="v1.0"
# 预训练任务类别
TASK_TYPE="token_level,span_level,seq_level"
# 预训练任务
PRETRAIN_TASK_LEVEL_NAME="gene_mask,gene_type,gene_taxonomy,prot_mask,prot_site,prot_homo,prot_domain,prot_taxonomy,prot_keyword"
...
# checkpoint的间隔step数保存一次(训练了n_gpu * 200,000个样本保存一个checkpoint)
save_steps=200000
...
cd ..
python -W ignore -m torch.distributed.launch --nnodes 1 --node_rank 0 --master_port 29997 --nproc_per_node=$nproc_per_node \
       run.py \
       --time_str $time_str \
       ...
       --gene_type_label_filepath ../label/$DATASET_NAME/$DATASET_TYPE/lucavirus_gene_type_span_level_label.txt \
       --prot_homo_label_filepath ../label/$DATASET_NAME/$DATASET_TYPE/lucavirus_prot_homo_span_level_label.txt \
       --prot_site_label_filepath ../label/$DATASET_NAME/$DATASET_TYPE/lucavirus_prot_site_span_level_label.txt \
       --prot_domain_label_filepath ../label/$DATASET_NAME/$DATASET_TYPE/lucavirus_prot_domain_span_level_label.txt \
       --gene_taxonomy_label_filepath ../label/$DATASET_NAME/$DATASET_TYPE/lucavirus_gene_taxonomy_seq_level_label.txt \
       --prot_taxonomy_label_filepath ../label/$DATASET_NAME/$DATASET_TYPE/lucavirus_prot_taxonomy_seq_level_label.txt \
       --prot_keyword_label_filepath ../label/$DATASET_NAME/$DATASET_TYPE/lucavirus_prot_keyword_seq_level_label.txt \
       ...
       --trained_checkpoint 3800000 \
       --model_dirpath ../models/lucavirus/v1.0/token_level,span_level,seq_level/lucavirus/20240815023346/checkpoint-step3800000
