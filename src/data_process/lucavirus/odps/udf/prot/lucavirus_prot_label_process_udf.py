...
@project: LucaVirus
@file: lucavirus_prot_label_process_udf.py
@desc: 蛋白质的label的name到index的转换，以及label整体的json组装
"""
...
class prot_label_process(BaseUDAF):
    def __init__(self):
        workaround_argv_issue("trembl_label_process_v2_udf.py")
        sys.path.insert(0, 'work/biopython-1.80-cp37.zip')

        protein_homo_span_level_label = {}
        for line in get_cache_file("sanyuan_lucavirus_prot_homo_span_level_label.txt"):
        ...
        for line in get_cache_file("sanyuan_lucavirus_prot_site_span_level_label.txt"):
        ...
        for line in get_cache_file("sanyuan_lucavirus_prot_domain_span_level_label.txt"):
        ...
        for line in get_cache_file("sanyuan_lucavirus_prot_taxonomy_seq_level_label.txt"):
        ...
        for line in get_cache_file("sanyuan_lucavirus_prot_keyword_seq_level_label.txt"):
    ...
    def terminate(self, buffer):
        ...
        for item1 in buffer.items():
            ...
            seq_level = item1[1]["seq_level"] if "seq_level" in item1[1] else None
            if seq_level is not None and "prot_keyword" in seq_level:
