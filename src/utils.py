...
def calc_detail_losses(losses, total_losses, total_steps, log_total_losses=None, log_total_steps=None):
    current_losses = {}
    for cur_losses in losses:
        for item1 in cur_losses.items():
            key1 = item1[0]
...
def download_trained_checkpoint_lucavirus(
    ...
    # assert llm_type in ["lucavirus", "lucavirus-gene", "lucavirus-prot", "lucavirus-mask"]
    # assert llm_task_level in ["token_level,span_level,seq_level", "token_level,span_level,seq_level", "token_level,span_level,seq_level", "token_level"]
    # assert llm_time_str in ["20240815023346", "20250118234004", "20250504090749", "20250113063529"]
    # assert llm_step in ["3800000", "3800000", "3800000", "1400000"]
    try:
        logs_file_names = ["logs.txt"]
        models_file_names = ["config.json", "pytorch.pth", "training_args.bin", "tokenizer/alphabet.pkl"]
