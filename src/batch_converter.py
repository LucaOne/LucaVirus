...
class BatchConverter(object):
    ...
    def __call_single__(self, batch_size, seq_types, seqs, seq_labels):
        ...
        for i, (label, seq_type, seq_str, seq_encoded) in enumerate(
            ...
            input_ids[i, int(self.alphabet.prepend_bos): seq_len + int(self.alphabet.prepend_bos)] = seq
            if self.alphabet.append_eos:
                input_ids[i, seq_len + int(self.alphabet.prepend_bos)] = self.alphabet.eos_idx

            if not self.no_position_embeddings:
                cur_len = int(self.alphabet.prepend_bos) + seq_len + int(self.alphabet.append_eos)
