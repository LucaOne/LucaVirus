...
try:
    ...
    from utils import to_device, get_lr, calc_loss, calc_loss_index, writer_info_tb, \
        print_batch_input, print_batch_output, print_batch, lcm, write_processed_sample_ids, calc_detail_losses, calc_avg_loss
                            ...
                            if global_step % args.loss_logging_steps == 0:
                                print("Training, Epoch: %d, Batch: %d, Sample Num: %d, Cur Loss: %.08f, Log Avg Loss: %.08f, Global Avg Loss: %.08f\n" %
                                      (
                                          ...
                                          done_sample_num,
                                          cur_loss,
                                          logging_loss / lcm(args.loss_logging_steps, args.gradient_accumulation_steps),
                                          total_loss/global_step
                                      )
                            )
                            if global_step == 1 or global_step % args.loss_logging_steps == 0:
